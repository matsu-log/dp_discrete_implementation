...
text \<open>These two lemmas are defined to be used in proofs involving induction on the fixed point of a function.\<close>
...
text \<open>We define an equivalent while function using the\<^locale>\<open>loop_spmf\<close>, and use it to prove losslessness.\<close>
