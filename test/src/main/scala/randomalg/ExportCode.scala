...
def map_prod[A, B, C, D](f : A => B, g : C => D, x2 : (A, C)) : (B, D) =
  (f, g, x2) match {
...
object power {
  implicit def `Real.power_real` : power[Real.real] = new power[Real.real] {
    val `Groups.times` = (a : Real.real, b : Real.real) =>
      Real.times_reala(a, b)
...
object cancel_comm_monoid_add {
  implicit def
    `Nat.cancel_comm_monoid_add_nat` : cancel_comm_monoid_add[Nat.nat] = new
    cancel_comm_monoid_add[Nat.nat] {
    val `Groups.zero` = Nat.zero_nata
    val `Groups.minus` = (a : Nat.nat, b : Nat.nat) => Nat.minus_nata(a, b)
    val `Groups.plus` = (a : Nat.nat, b : Nat.nat) => Nat.plus_nata(a, b)
  }
...
object comm_semiring_0_cancel {
  implicit def
    `Nat.comm_semiring_0_cancel_nat` : comm_semiring_0_cancel[Nat.nat] = new
    comm_semiring_0_cancel[Nat.nat] {
    val `Groups.minus` = (a : Nat.nat, b : Nat.nat) => Nat.minus_nata(a, b)
    val `Groups.zero` = Nat.zero_nata
    val `Groups.plus` = (a : Nat.nat, b : Nat.nat) => Nat.plus_nata(a, b)
    val `Groups.times` = (a : Nat.nat, b : Nat.nat) => Nat.times_nata(a, b)
...
object comm_semiring_1_cancel {
  implicit def
    `Nat.comm_semiring_1_cancel_nat` : comm_semiring_1_cancel[Nat.nat] = new
    comm_semiring_1_cancel[Nat.nat] {
    val `Groups.one` = Nat.one_nata
    val `Groups.times` = (a : Nat.nat, b : Nat.nat) => Nat.times_nata(a, b)
    val `Groups.zero` = Nat.zero_nata
    val `Groups.minus` = (a : Nat.nat, b : Nat.nat) => Nat.minus_nata(a, b)
...
trait divide_trivial[A] extends Groups.one[A] with Groups.zero[A] with divide[A]
  {
}
object divide_trivial {
  implicit def `Euclidean_Rings.divide_trivial_nat` : divide_trivial[Nat.nat] =
    new divide_trivial[Nat.nat] {
    val `Rings.divide` = (a : Nat.nat, b : Nat.nat) =>
      Euclidean_Rings.divide_nata(a, b)
    val `Groups.zero` = Nat.zero_nata
    val `Groups.one` = Nat.one_nata
...
object semiring_no_zero_divisors_cancel {
  implicit def
    `Euclidean_Rings.semiring_no_zero_divisors_cancel_nat` : semiring_no_zero_divisors_cancel[Nat.nat]
...
trait semidom_divide[A]
  extends divide_trivial[A] with semidom[A] with
    semiring_no_zero_divisors_cancel[A]
  {
}
...
trait semiring_modulo_trivial[A]
  extends divide_trivial[A] with semiring_modulo[A] {
}
object semiring_modulo_trivial {
  implicit def
    `Euclidean_Rings.semiring_modulo_trivial_nat` : semiring_modulo_trivial[Nat.nat]
    = new semiring_modulo_trivial[Nat.nat] {
    val `Rings.modulo` = (a : Nat.nat, b : Nat.nat) =>
      Euclidean_Rings.modulo_nata(a, b)
    val `Groups.times` = (a : Nat.nat, b : Nat.nat) => Nat.times_nata(a, b)
    val `Groups.minus` = (a : Nat.nat, b : Nat.nat) => Nat.minus_nata(a, b)
...
trait semidom_modulo[A]
  extends algebraic_semidom[A] with semiring_modulo_trivial[A] {
}
           ...
           (s == BigInt(0) match { case true => ((- r), BigInt(0))
             case false => ((- r) - BigInt(1), l - s) })
                    ...
                    case false => (l == BigInt(0) match {
                                    ...
                                    case false => Product_Type.apsnd[BigInt,
                              BigInt,
                              BigInt](((a : BigInt) => (- a)),
                                       (k < BigInt(0) match {
 case true => ((k: BigInt) => (l: BigInt) =>
...
def of_nat[A : Rings.semiring_1](n : nat) : A =
  (equal_nata(n, zero_nata) match { case true => Groups.zero[A]
    case false => {
                    val (m, q) =
                      Euclidean_Rings.divmod_nat(n,
          Code_Numeral.nat_of_integer(BigInt(2))) : ((nat, nat))
                    val ma =
                      Groups.times[A](Num.numeral[A](Num.Bit0(Num.One())),
                                       of_nat[A](m)) : A;
                    (equal_nata(q, zero_nata) match { case true => ma
                      case false => Groups.plus[A](ma, Groups.one[A]) })
                  }
    })
...
def normalize(p : (Int.int, Int.int)) : (Int.int, Int.int) =
  (Int.less_int(Int.zero_int, Product_Type.snd[Int.int, Int.int](p)) match {
    case true => {
                   val a =
    ...
    case false => (Int.equal_int(Product_Type.snd[Int.int, Int.int](p),
                                  Int.zero_int) match {
                    case true => (Int.zero_int, Int.one_int)
                    case false => {
                                    val a =
                                      Int.uminus_int(GCD.gcd_int(Product_Type.fst[Int.int,
   Int.int](p),
                          Product_Type.snd[Int.int, Int.int](p))) : Int.int;
                                    (Euclidean_Rings.divide_int(Product_Type.fst[Int.int,
  Int.int](p),
                         a),
                                      Euclidean_Rings.divide_int(Product_Type.snd[Int.int,
...
def bind_rai[A, B](m : Coin_Space.coin_stream =>
                    ...
                    bs : Coin_Space.coin_stream) : Option[(B,
                    Coin_Space.coin_stream)]
  =
...
def coin_rai(bs : Coin_Space.coin_stream) : Option[(Boolean,
             Coin_Space.coin_stream)]
  =
  Some[(Boolean,
         Coin_Space.coin_stream)]((Coin_Space.chd(bs), Coin_Space.ctl(bs)))

def return_rai[A](x : A,
                   bs : Coin_Space.coin_stream) : Option[(A,
                   Coin_Space.coin_stream)]
  =
...
def Rep_random_alg[A](x0 : random_alg[A]) : Coin_Space.coin_stream =>
      Option[(A, Coin_Space.coin_stream)]
  =
  x0 match {
...
def fast_dice_roll_ra(n : Nat.nat, v : Nat.nat,
                       c : Nat.nat) : Randomized_Algorithm.random_alg[Nat.nat]
  =
  (Nat.less_eq_nat(n, v) match {
    case true => (Nat.less_nat(c, n) match {
                   case true => Randomized_Algorithm.return_ra[Nat.nat](c)
                   case false => fast_dice_roll_ra(n, Nat.minus_nata(v, n),
            Nat.minus_nata(c, n))
                   })
    case false => Randomized_Algorithm.bind_ra[Boolean,
        Nat.nat](Randomized_Algorithm.coin_ra,
                 ...
                 v),
                             ...
                             c),
              (b match { case true => Nat.one_nata case false => Nat.zero_nata
                })))))
    })
...
def bernoulli_rat_ra(n : Nat.nat,
                      d : Nat.nat) : Randomized_Algorithm.random_alg[Boolean]
  =
  (Nat.equal_nata(d, Nat.zero_nata) match {
    case true => Randomized_Algorithm.return_ra[Boolean](false)
    case false => Randomized_Algorithm.bind_ra[Nat.nat,
        Boolean](fast_uniform_ra(d),
                  ((x : Nat.nat) =>
                    Randomized_Algorithm.return_ra[Boolean](Nat.less_nat(x,
                                  n))))
    })

def bernoulli_exp_minus_rat_from_0_to_1_loop_ra(p : Rat.rat,
         k : Nat.nat) : Randomized_Algorithm.random_alg[Nat.nat]
  =
  ...
  ((a : Boolean) =>
    (a match {
      case true => bernoulli_exp_minus_rat_from_0_to_1_loop_ra(p,
                        Nat.plus_nata(k, Nat.one_nata))
      case false => Randomized_Algorithm.return_ra[Nat.nat](k) })))

def bernoulli_exp_minus_rat_from_0_to_1_ra(p : Rat.rat) : Randomized_Algorithm.random_alg[Boolean]
  =
  Randomized_Algorithm.bind_ra[Nat.nat,
                                Boolean](bernoulli_exp_minus_rat_from_0_to_1_loop_ra(p,
      Nat.one_nata),
  ((k : Nat.nat) =>
    (! (Rings.dvd[Nat.nat](Code_Numeral.nat_of_integer(BigInt(2)), k)) match {
      case true => Randomized_Algorithm.return_ra[Boolean](true)
      case false => Randomized_Algorithm.return_ra[Boolean](false) })))

def bernoulli_exp_minus_rat_loop_ra(k : Nat.nat) : Randomized_Algorithm.random_alg[Boolean]
  =
  (Nat.less_eq_nat(Nat.one_nata, k) match {
    case true => Randomized_Algorithm.bind_ra[Boolean,
       Boolean](bernoulli_exp_minus_rat_from_0_to_1_ra(Rat.one_rat),
                 ((b : Boolean) =>
                   (b match {
                     case true => bernoulli_exp_minus_rat_loop_ra(Nat.minus_nata(k,
  Nat.one_nata))
                     case false => Randomized_Algorithm.return_ra[Boolean](false)
                     })))
    case false => Randomized_Algorithm.return_ra[Boolean](true) })

def bernoulli_exp_minus_rat_ra(p : Rat.rat) : Randomized_Algorithm.random_alg[Boolean]
  =
  (Rat.less_rat(p, Rat.zero_rat) match {
    case true => Randomized_Algorithm.return_ra[Boolean](true)
    case false => (Rat.less_eq_rat(Rat.zero_rat, p) &&
                     Rat.less_eq_rat(p, Rat.one_rat) match {
                    case true => bernoulli_exp_minus_rat_from_0_to_1_ra(p)
                    case false => Randomized_Algorithm.bind_ra[Boolean,
                        Boolean](bernoulli_exp_minus_rat_loop_ra(Int.nat(Rat.floor_rat(p))),
                                  ((b : Boolean) =>
                                    (b match {
                                      case true => bernoulli_exp_minus_rat_from_0_to_1_ra(Rat.minus_rat(p,
                         Rat.of_int(Rat.floor_rat(p))))
                                      case false => Randomized_Algorithm.return_ra[Boolean](b)
                                      })))
                    })
    })

def discrete_laplace_rat_unit_loop2_ra(v : Nat.nat) : Randomized_Algorithm.random_alg[Nat.nat]
  =
  Randomized_Algorithm.bind_ra[Boolean,
                                Nat.nat](bernoulli_exp_minus_rat_ra(Rat.one_rat),
  ((a : Boolean) =>
    (Product_Type.equal_bool(a, false) match {
      case true => Randomized_Algorithm.return_ra[Nat.nat](v)
      case false => discrete_laplace_rat_unit_loop2_ra(Nat.plus_nata(v,
                              Nat.one_nata))
      })))

def discrete_laplace_rat_unit_loop1_ra(t : Nat.nat) : Randomized_Algorithm.random_alg[Nat.nat]
  =
  Randomized_Algorithm.bind_ra[Nat.nat,
 ...
 Code_Target_Nat.int_of_nat(t))),
    ((d : Boolean) =>
      (d match { case true => Randomized_Algorithm.return_ra[Nat.nat](u)
        case false => discrete_laplace_rat_unit_loop1_ra(t) })))))

def discrete_laplace_rat_unit_ra(t : Nat.nat) : Randomized_Algorithm.random_alg[Nat.nat]
  =
  Randomized_Algorithm.bind_ra[Nat.nat,
...
def discrete_laplace_rat_ra(t : Nat.nat,
                             s : Nat.nat) : Randomized_Algorithm.random_alg[Int.int]
  =
  Randomized_Algorithm.bind_ra[Nat.nat,
    ...
    ((b : Boolean) =>
      {
        ...
        (! b &&
           Int.equal_int(Code_Target_Nat.int_of_nat(xa), Int.zero_int) match {
          case true => discrete_laplace_rat_ra(t, s)
          case false => (b match {
                          case true => Randomized_Algorithm.return_ra[Int.int](Int.uminus_int(Code_Target_Nat.int_of_nat(xa)))
                          case false => Randomized_Algorithm.return_ra[Int.int](Code_Target_Nat.int_of_nat(xa))
                          })
          })
      }))))
