...
proof -
  ...
  have "((=) ===> rel_spmf_of_ra) (\<lambda>pair. loop1_alt (fst pair) (snd pair)) (\<lambda>pair. loop1_alt_ra (fst pair) (snd pair))"
    ...
    unfolding curry_def apply(simp)
  proof -
    have " (\<lambda>loop1_alt (p, k). (case quotient_of p of (n, d) \<Rightarrow> bernoulli_rat (nat n) (nat (d * int k))) \<bind> (\<lambda>a. if a then loop1_alt (p, k + 1) else return_spmf k))
